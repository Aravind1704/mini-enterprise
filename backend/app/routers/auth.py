...
from app.database import get_db
from app.models.user import User
from app.models.subscription import PasswordResetToken
...
def forgot_password(email: str, db: Session = Depends(get_db)):
    ...
    token = create_password_reset_token(user.email, expires_minutes=15)
    pr = PasswordResetToken(user_id=user.id, token=token, used=False, expires_at=datetime.utcnow() + timedelta(minutes=15))
    db.add(pr); db.commit()
    send_password_reset_email(email=user.email, reset_token=token, user_name=user.name)
...
def reset_password(token: str, new_password: str, db: Session = Depends(get_db)):
    ...
    pr = db.query(PasswordResetToken).filter(PasswordResetToken.token == token, PasswordResetToken.used == False).first()
